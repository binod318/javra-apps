...
namespace Enza.UTM.BusinessAccess.Services
{
    public class TestService : ITestService
    {
        ...
        public async Task<IEnumerable<TestLookup>> GetLookupAsync(string cropCode, string breedingStationCode, string testTypeMenu = null)
        {
            return await repository.GetLookupAsync(cropCode, breedingStationCode,testTypeMenu);
        }
                    ...
                    try
                    {
                        ...
                        foreach (var dataPerTest in dataPerTests)
                        {
                            try
                            {
                                ...
                                if (!setColResp.Status.EqualsIgnoreCase("1"))
                                {
                                    if(test.StatusCode < 650)
                                    {
                                        ...
                                        await SendAddColumnErrorEmailAsync(cropCode, test.BrStationCode, test.PlatePlanName,slotDetail?.Remarks,setColResp.ErrorType, setColResp.Message);
                                        //update test status
                                        await repository.UpdateTestStatusAsync(new UpdateTestStatusRequestArgs
                                        {
                                            TestId = testId,
        ...
        private async Task SendAddColumnErrorEmailAsync(string cropCode, string brStationCode, string platePlanName,string remark, string errorType, string missingColumns)
        {
            ...
            if (errorType.EqualsIgnoreCase("MissingColumn") || errorType.EqualsIgnoreCase("invalidResponse"))
            {
                //get test complete email body template
                var testCompleteBoy = EmailTemplate.GetColumnSetErrorEmailTemplate("MissingColumn");
            ...
            //else we have error type LimitExceeded
            else
            {
                //get test complete email body template
                //this is for error type limit exceeded
                var testCompleteBoy = EmailTemplate.GetColumnSetErrorEmailTemplate("2GB");
                body = Template.Render(testCompleteBoy, new
                {
                    PlatePlanName = platePlanName,
        ...
        public async Task<byte[]> PlatePlanResultToExcelAsync(int testID, bool? withControlPosition)
        {
            ...
            //create excel
            //here first table contains data that needs to be created and second table contains the dead material with palte name and well position
            var createExcel = CreateExcelFile(data);
            //apply formating 
            CreateFormatting(createExcel,data.Tables[0].Columns.Count);

            //apply dead material position formating
        ...
        public async Task<byte[]> TestToExcelAsync(int testID, bool? withControlPosition)
        {
            //get data
            var data = await repository.TestToExcelAsync(testID, withControlPosition);
            //create excel
            var createExcel = CreateExcelFile(data);
        ...
        {            
            ...
            if (tos.Any())
            {
                ...
                var subject = $"Folder {platePlanName} changed to completed".AddEnv();
                var sender = ConfigurationManager.AppSettings["TestCompletedEmailSender"];
                await emailService.SendEmailAsync(sender, tos, subject, body);
        ...
        public async Task<PlateValidationResponse> CheckPlateMultipleOf4Async(int testID)
        {
            return await repository.CheckPlateMultipleOf4Async(testID);
        }
        ...
        {
            var deadMaterialPlateWell = new List<WellWithPlate>();
            //create workbook 
            ...
            {
                //now get data from table1.
                foreach (DataRow dr in data.Tables[1].Rows)
                {
                    deadMaterialPlateWell.Add(new WellWithPlate()
                    {
                        WellPosition = dr["Position"].ToText(),
                        Platename = dr["PlateName"].ToText()
                    });
                }
            ...
            //create data
            var rowNr = 1;
            bool applyDeadMaterialStype = false;

            foreach (DataRow dr in data.Tables[0].Rows)
            {
                applyDeadMaterialStype = false;
                var row = sheet1.CreateRow(rowNr);
                var plateName = "";
                var wellPosition = "";
                if (data.Tables[0].Columns.Contains("PlateName"))
                {
                    plateName = data.Tables[0].Rows[rowNr - 1]["PlateName"].ToText();
                }
                if (data.Tables[0].Columns.Contains("Well"))
                {
                    wellPosition = data.Tables[0].Rows[rowNr - 1]["Well"].ToText();
                }
                if (deadMaterialPlateWell.Any(x => x.Platename == plateName && x.WellPosition == wellPosition))
                {
                    applyDeadMaterialStype = true;
                }
        ...
        private void setCellStyle(XSSFWorkbook workbook, ICell cell)
        {
            ...
            fCellStyle.FillForegroundColor = IndexedColors.Red.Index;
