            ...
            using (var client = new RestClient(PHENOME_BASE_SVC_URL))
            {
                ...
                foreach (var _config in config)
                {
                    try
                    {
                        ...
                        var groupedDataTotal = getDHToCreate.GroupBy(g => g.RefExternal);
                        var groupedDataByRefExt = getDHToCreate.GroupBy(g => g.RefExternal);

                        foreach (var _groupdeDataByRefExt in groupedDataByRefExt)
                        {
                            var batchSize = 10;
                            var groupedDataBatch = _groupdeDataByRefExt.BatchBy(batchSize);
                            var key = _groupdeDataByRefExt.Key;
                            var iteration = 0;
                            foreach (var _groupedDataBatch in groupedDataBatch)
                            {
                                iteration++;
                                LogInfo($"Create DH0 and DH1 to phenome started with BatchSize {batchSize} with iteration {iteration}");
                                ...
                                try
                                {
                                    ...
                                    //1. get columns
                                    LogInfo($"Get Columns from Phenome for field {key}");
                                    var columnsResp = await PrepareFilterGrid(client, setGridURL, key, gridID, "{}", "28");

                                    //2. if any of two columns is not found add it on grid  
                                    LogInfo($"Set required Columns from Phenome for field {key}");
                                    var requiredColumns = new List<string>();
                                    requiredColumns.Add("gid");
                                    requiredColumns.Add("name");
                                    var addColumnsResp = await AddColumns(client, setOrderURL, keyvalues, requiredColumns, columnsResp, key, "6");
                                    if (!addColumnsResp)
                                    {
                                    ...
                                    LogInfo($"Set Grid with filter value on field {key}");
                                    var filtervalue = string.Join(",", _groupedDataBatch.Select(x => x.DHProposedName).ToList());
                                    var filterString = "{\"name\":\"" + filtervalue + "\"}";
                                    var jsonresp = await PrepareFilterGrid(client, setGridURL, key, gridID, filterString, "28");
                                    if (jsonresp.Status.ToText().EqualsIgnoreCase("1"))
                                    {
                                        ...
                                        if (nameCol == null)
                                        {
                                            LogError($"Selection name column not found on field { key}");
                                            errorMessage.Add(new ExecutableError
                                            {
                                                Success = false,
                                                CropCode = _config.CropCode,
                                                ErrorType = "Exception",
                                                ErrorMessage = $"Selection name column not found on field { key}"
                                        ...
                                        LogInfo($"Check if DH0 is already created in field {key}");
                                        createdDH0Data = await FetchCreatedSelectionData(client, fetchDataURL, gridID, key, "28", requiredColumns);

                                        //create data if it is not already created
                                        var tobeCreated = _groupedDataBatch.Select(x => x).Where(x => !createdDH0Data.Any(y => y.ProposedName == x.DHProposedName)).ToList();
                                        if (tobeCreated.Any())
                                        {
                                            LogInfo($"Create Selection for DHO on field {key}");
                                            foreach (var _toBeCreated in tobeCreated)
                                            {
                                                //create DH0 record
                                                var createSelectionResp = await CreateDH(client, createSelectionURL, _toBeCreated.Materialkey, key, _toBeCreated.DHProposedName, _toBeCreated.FieldEntityType, dH0MethodID);
                                                if (!createSelectionResp.Status.EqualsIgnoreCase("1"))
                                                {
                                                    errorMessage.Add(new ExecutableError
                                                    {
                                                        ...
                                                        CropCode = _config.CropCode,
                                                        ErrorType = "data",
                                                        ErrorMessage = $"Unable to create selection (Double Haploid) with methodID: {dH0MethodID} () on fieldID: {key} with name: {_toBeCreated.DHProposedName}"
                                            ...
                                            //1. get columns
                                            jsonresp = await PrepareFilterGrid(client, setGridURL, key, gridID, filterString, "28");
                                            if (jsonresp.Status.ToText().EqualsIgnoreCase("1"))
                                            {
                                                LogInfo($"Get Created Selection (Double Haploid) data on field {key}.");
                                        ...
                                        if (dataToMove.Any())
                                        {
                                            ...
                                            moveToTargetField = await MoveDH0ToTargetField(client, dragDropURL, _config.DHFieldSetSetID, selectionIDS, count, key);
                                        }
                                        ...
                                        if ((count > 0 && JsonConvert.DeserializeObject<PhenomeResponse>(moveToTargetField).Status == "1") || count <= 0)
                                        {
                                            ...
                                            if (jsonresp1.Status.ToText().EqualsIgnoreCase("1"))
                                            {
                                                ...
                                                if (createdDH1.Status.ToText().EqualsIgnoreCase("1"))
                                                {
                                                    ...
                                                    var tobeCreatedDH1 = _groupedDataBatch.Select(x => x).Where(x => !createdDH1Data.Any(y => y.ProposedName == x.DHProposedName)).ToList();
                                    ...
                                    {
                                        LogError($"Unable to set grid on field { key}");
                                        errorMessage.Add(new ExecutableError
                                        {
                                            Success = false,
                                            CropCode = _config.CropCode,
                                            ErrorType = "Exception",
                                            ErrorMessage = $"Unable to set grid on field { key}"
                        ...
                        LogInfo($"Creating DH0 and DH1 complete");
                    }
                    catch (Exception ex)
                    {
                        LogError(ex);
            ...
            if (configs.Any())
            {
                ...
                foreach (var config in configs)
                {
                    ...
                    try
                    {
                        ...
                        var totaldata = await s2SRepository.GetDH1DetailsAsync(config.TestID);
                        var batchSize = 200;
                        if (totaldata.Any())
                        {
                            var databyBatch = totaldata.BatchBy(batchSize);
                            LogInfo($"Calling Cordys service to store DH1 information with Batch size { batchSize } materials.");
                            var loopCount = 0;
                            foreach (var data in databyBatch)
                            {
                                try
                                {
                                    loopCount++;
                                    ...
                                    })
                                    {
                                        ...
                                        if (!resp.Success)
                                        {
                                            ...
                                            uel.LogError(new Exception(msg), out string logID);
                                        }
                                        else
                                        {
                                            ...
                                            LogInfo($"Calling Cordys service to store DH1 information is completed successfully for iteration {loopCount}.");
                                        }
                                    }
                                }
                                catch (Exception ex)
