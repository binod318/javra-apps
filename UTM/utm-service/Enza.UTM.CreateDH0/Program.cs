            ...
            try
            {
                ...
                using (var scope = Container.BeginLifetimeScope())
                {
                    var success = AsyncHelper.RunSync(async () =>
                    {
                        ...
                        //call cordys service if any data is left to sync to send DH1 information of all crops to cordys
                        var sendtoCordys1 = await service.StoreGIDinCordysAsync();

                        //call service to create data to phenome 
                        var CreateDHresponse = await service.CreateDHAsync();
                        
                        //Call cordys service to Send DH1 information of created records of all crops to Cordys
                        var sendtoCordys2 = await service.StoreGIDinCordysAsync();
                        
                        //concat eror message and exception to send email based on error type we get.

                        response.AddRange(sendtoCordys1);
                        response.AddRange(CreateDHresponse);
                        response.AddRange(sendtoCordys2);
