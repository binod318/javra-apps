...
namespace Enza.UTM.DataAccess.Data.Repositories
{
    public class ThreeGBRepository : Repository<object>, IThreeGBRepository
    {
        ...
        {
            var query = "SELECT TOP 1 RIGHT(PeriodName,2) FROM [Period] WHERE GETDATE() BETWEEN StartDate and EndDate";
            var data = await DbContext.ExecuteReaderAsync(query,
                reader=>new PlanPeriod
                {
                    PeriodName = reader.Get<string>(0),
                });
            var weekNumber = data.FirstOrDefault().PeriodName;

            var cullture = CultureInfo.CurrentCulture;
            var datetime = DateTime.Now;
            //var currentWeekNr = cullture.Calendar.GetWeekOfYear(datetime, CalendarWeekRule.FirstFullWeek, DayOfWeek.Monday);
            var weekNr = Convert.ToInt32($"{datetime.Year}{weekNumber}");
