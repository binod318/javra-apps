    ...
    public class TestRepository : Repository<TestLookup>, ITestRepository
    {
        ...
        public async Task<IEnumerable<TestLookup>> GetLookupAsync(string cropCode, string breedingStationCode, string testTypeMenu = null)
        {
            return await DbContext.ExecuteReaderAsync(DataConstants.PR_GET_TESTS_LOOKUP, CommandType.StoredProcedure, args => 
            {
                ...
                args.Add("@TestTypeMenu", testTypeMenu);
            }, reader => new TestLookup
            {
                ...
                TestProtocolID = reader.Get<int?>(24),
                SiteID = reader.Get<int?>(25),                
                SampleType = reader.Get<string>(26)
            });
        }
        ...
        public async Task<bool> UpdateTest(UpdateTestArgs updatetestArgs)
        {
            await DbContext.ExecuteNonQueryAsync(DataConstants.PR_UPDATE_TEST, CommandType.StoredProcedure,
              args =>
              {
                  ...
                  args.Add("@SiteID", updatetestArgs.SiteID);
              });
            return true;
        }
        ...
        public async Task<DataSet> TestToExcelAsync(int testID, bool? withControlPosition)
        {
        ...
        public async Task<PlateValidationResponse> CheckPlateMultipleOf4Async(int testID)
        {
            ...
            var result =  await DbContext.ExecuteReaderAsync(DataConstants.PR_CHECK_PLATE_VALIDATION, CommandType.StoredProcedure,
                args =>
                {
                    args.Add("@TestID", testID);
                },
                reader => new PlateValidationResponse
                {
                    Success = reader.Get<bool>(0),
                    Message = reader.Get<string>(1)
                });
