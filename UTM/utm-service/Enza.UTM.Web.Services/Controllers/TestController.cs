...
namespace Enza.UTM.Web.Services.Controllers
{
    ...
    public class TestController : BaseApiController
    {
        ...
        //[Authorize(Roles = AppRoles.PUBLIC)]
        public async Task<IHttpActionResult> GetLookup(string cropCode, string breedingStationCode, string testTypeMenu=null)
        {
            var testLookups = await testService.GetLookupAsync(cropCode, breedingStationCode,testTypeMenu);
        ...
        public async Task<IHttpActionResult> ReservePlatesInLims([FromBody] ReservePlatesInLIMSRequestArgs args)
        {
            if (args == null)
                return InvalidRequest("Please specify required parameters.");
            var plateValidationResponse = new PlateValidationResponse();

            //check if plates are multiple of 4 in request or not
            if(!args.Forced)
            {
                plateValidationResponse = await testService.CheckPlateMultipleOf4Async(args.TestID);
                if(!plateValidationResponse.Success)
                {
                    return Ok(new
                    {
                        plateValidationResponse.Success,
                        plateValidationResponse.Message,
                        TestID = args.TestID,
                        StatusCode = 100
                    });
                }
        ...
        [HttpGet]
        [Route("TestToExcel")]        
        public async Task<IHttpActionResult> TestToExcel(int testID, bool? withControlPosition = null)
        {
            var data = await testService.TestToExcelAsync(testID, withControlPosition);
            var result = new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new ByteArrayContent(data)
            };
