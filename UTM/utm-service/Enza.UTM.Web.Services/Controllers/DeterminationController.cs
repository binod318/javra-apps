...
namespace Enza.UTM.Web.Services.Controllers
{
    ...
    public class DeterminationController : BaseApiController
    {
        ...
        [Route("getLeafDiskDetermination")]
        [HttpGet]
        public async Task<IHttpActionResult> GetLeafDiskDetermination([FromUri]string CropCode)
        {
            var result = await determinationService.GetLeafDiskDeterminationAsync(CropCode);
            return Ok(result);
        }
        [Route("getSeedHealthDetermination")]
        [HttpGet]
        public async Task<IHttpActionResult> GetSeedHealthDetermination([FromUri]string CropCode)
        {
            var result = await determinationService.GetSeedHealthDeterminationAsync(CropCode);
            return Ok(result);
        }
    }
}
