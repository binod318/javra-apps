            ...
            try
            {
                ...
                using (var scope = Container.BeginLifetimeScope())
                {
                    var success = AsyncHelper.RunSync(async () =>
                    {
                        ...
                        var shService = scope.Resolve<ISeedHealthService>();
                        ...
                        //send Seed health result
                        var sendResultSH = await shService.SendResult().ExecuteSafe(err =>
                        {
                            ErrorLog(err);
                        });

                        if (!sendResult2GB || !sendResultRDT || !sendResultSH)
                        {
                        ...
                        if (sendResultRDT && sendResult2GB && sendResultSH)
                            return true;
                        else
                            return false;
                    });
