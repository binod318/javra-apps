            ...
            foreach (var item in Determinations)
            {
                foreach (var data in item.Materials)
                {
                    foreach(var score in data.Scores)
                    {
                        ...
                        // for flow 2 either SusceptibilityPer or ResistancePer or Score
                        if (score.Key.ToLower().Contains("susceptibilityper") || score.Key.ToLower().Contains("resistantper"))
                        {
                            //handles U (Undetermined) value on Percentage : store undermined value as character in Score column
                            var isNumeric = int.TryParse(score.Value, out _);

                            if(isNumeric)
                                dr["Percentage"] = score.Value;
                            else
