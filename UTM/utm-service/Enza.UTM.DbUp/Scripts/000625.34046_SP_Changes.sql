...
/*
EXEC  PR_Get_Files 'ON', 'NLEN',NULL,'SeedHealth'
*/
...
(
	...
	@TestID INT = NULL,
	@TestTypeMenu NVARCHAR(MAX) = NULL
) AS
BEGIN
	...
	SELECT                  
		F.FileID, 	                        
		F.CropCode, 
		FileTitle = T.TestName, --F.FileTitle, 
		F.UserID, 
		F.ImportDateTime,
		T.TestID,
		T.TestTypeID,
	...
	JOIN TestType TT ON TT.TestTypeID = T.TestTypeID
						AND TT.TestTypeID >= CASE 
												WHEN ISNULL(@TestTypeMenu,'') = '' THEN 1 
												WHEN @TestTypeMenu = 'UTMGeneral' THEN 1
												WHEN @TestTypeMenu = 'RDT' THEN 8
												WHEN @TestTypeMenu = 'LeafDisk' THEN 9
												WHEN @TestTypeMenu = 'SeedHealth' THEN 10
											END
						AND TT.TestTypeID <= CASE 
												WHEN ISNULL(@TestTypeMenu,'') = '' THEN 100
												WHEN @TestTypeMenu = 'UTMGeneral' THEN 7
												WHEN @TestTypeMenu = 'RDT' THEN 8
												WHEN @TestTypeMenu = 'LeafDisk' THEN 9
												WHEN @TestTypeMenu = 'SeedHealth' THEN 10
											END
...
-- =============================================  
-- EXEC [PR_GetTestsLookup] 'ON', 'NLEN', 'leafdisk'  
