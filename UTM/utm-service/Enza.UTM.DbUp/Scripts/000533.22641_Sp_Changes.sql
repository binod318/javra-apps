...
BEGIN
	...
	ORDER BY CTE.[SampleID], CTE.MaterialPlantID
...
(
	...
	(
		...
		WHERE [Key] IN('QRCode','SampleName') 
		AND ST.TestID = @TestID
	) S ON S.SampleID = T.SampleID
	WHEN MATCHED THEN
	UPDATE SET 
		ReferenceCode = CASE WHEN S.[Key] = 'QRCode' THEN S.[Value] ELSE T.ReferenceCode END, 
		SampleName = CASE WHEN S.[Key] = 'SampleName' THEN ISNULL(S.[Value],T.SampleName) ELSE T.Samplename END;
