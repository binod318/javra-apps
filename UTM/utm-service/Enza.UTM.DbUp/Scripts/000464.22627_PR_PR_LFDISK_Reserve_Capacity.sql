DROP PROCEDURE IF EXISTS [PR_PR_LFDISK_Reserve_Capacity]
GO
/*
...
CREATE PROCEDURE [dbo].[PR_PR_LFDISK_Reserve_Capacity]
(
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE P.PeriodID = @PeriodID AND TestProtocolID = @TestProtocolID AND (S.StatusCode = 200 OR (S.StatusCode = 100  AND ISNULL(S.AlreadyApproved,0) = 1));
		...
		BEGIN TRANSACTION;
			...
			IF(@InRange = 1) BEGIN
				INSERT INTO Slot(SlotName, PeriodID, StatusCode, CropCode, MaterialTypeID, RequestUser, RequestDate, PlannedDate, BreedingStationCode, Remark,TestTypeId,AlreadyApproved)
				VALUES(@SlotName,@PeriodID,200,@CropCode,@MaterialTypeID,@User,GETDATE(),@PlannedDate,@BreedingStationCode, @Remark,@TestTypeID,1);
			...
			ELSE IF(@Forced = 1 AND @InRange = 0) BEGIN				
				--create logic here....				
				INSERT INTO Slot(SlotName, PeriodID, StatusCode, CropCode, MaterialTypeID,  RequestUser, RequestDate, PlannedDate, BreedingStationCode,Remark,TestTypeID)
				VALUES(@SlotName, @PeriodID, 100, @CropCode, @MaterialTypeID,  @User, GETDATE(), @PlannedDate,  @BreedingStationCode,  @Remark, @TestTypeID);
