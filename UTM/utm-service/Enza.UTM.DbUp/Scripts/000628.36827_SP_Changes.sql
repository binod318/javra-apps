...
ALTER PROCEDURE [dbo].[PR_LFDISK_GetPlanApprovalListBySlotForLAB]
(
	@SiteID INT,
	...
	JOIN Slot T4 ON T4.SlotID = T1.SlotID AND T4.SiteID = ISNULL(@SiteID,0) AND T4.TestTypeID = 9
	JOIN 
	(
	   SELECT 
		  SlotID,
...
(
	...
	SELECT
	   ...
	   T2.StatusCode
    FROM ReservedCapacity T1
    JOIN Slot T2 ON T2.SlotID = T1.SlotID AND T2.TestTypeID in (1,2)
    JOIN @Periods P ON P.PeriodID = T2.PeriodID
	...
	FROM @Periods P 
	CROSS JOIN TestProtocol TP
	WHERE TP.Isolated = 0 AND TP.TestTypeID IN (1,2) ;	
	...
	FROM TestProtocol TP
	JOIN TestType TT On TT.TestTypeID = TP.TestTypeID
	WHERE TT.DeterminationRequired = 1 AND TT.TestTypeID IN (1,2);
	...
	(
		...
		JOIN [Period] P ON P.PeriodID = S.PeriodID OR S.ExpectedDate BETWEEN P.StartDate AND P.EndDate AND S.TestTypeID IN (1,2)
