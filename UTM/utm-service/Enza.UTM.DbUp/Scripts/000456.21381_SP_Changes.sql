DROP PROCEDURE IF EXISTS PR_RDT_UpdateTestStatus
GO
/*
...
CREATE PROCEDURE PR_RDT_UpdateTestStatus
(
	@TestID INT,
	@StatusCode INT
)
...
BEGIN
	...
	BEGIN
		UPDATE TestMaterialDetermination SET StatusCode = CASE WHEN StatusCode = 200 THEN 500 ELSE 400 END WHERE TestID = @TestID AND StatusCode IN (200, 300);
	END
...
(
	...
	@MaxSelectCrops NVARCHAR(MAX) = NULL
)
AS BEGIN
	SET NOCOUNT ON;

	DECLARE @DetColumns NVARCHAR(MAX),@DetColumnIDs NVARCHAR(MAX), @ImportedColumns NVARCHAR(MAX), @ImportedColumnIDs NVARCHAR(MAX), @AllColumns NVARCHAR(MAX),@DateColumns NVARCHAR(MAX), @LimsIDColumns NVARCHAR(MAX), @NrOfPlantsColumns NVARCHAR(MAX), @MaxSelectColumns NVARCHAR(MAX), @CropCode NVARCHAR(MAX);
	...
	BEGIN
		...
		--insert max select 
		SELECT @ShowMaxSelect = CASE WHEN ISNULL([value],'') <> '' THEN 1 ELSE 0 END
		FROM STRING_SPLIT(@MaxSelectCrops, ',') WHERE [Value] = @CropCode;

		IF(ISNULL(@ShowMaxSelect,0) = 1)
		BEGIN
			INSERT INTO @TblColumns(ColumnID, TraitID, ColumnLabel, ColumnType, ColumnNr,Updatable,DataType,ColumnOrder)
	...
	IF(ISNULL(@TestStatusCode,0) >= 450 AND ISNULL(@ImportLevel,'') <> 'PLT' AND ISNULL(@ShowMaxSelect,0) = 1)
	BEGIN
		SELECT 
		...
		(
			...
			(
				...
				(
					...
					FROM [TestMaterialDetermination] T1
					JOIN Material T2 ON T2.MaterialID = T1.MaterialID 
					WHERE T1.TestID = @TestID AND ISNULL(T1.StatusCode,100) NOT IN (200, 500) 
				) SRC 
				PIVOT
				(
...
(	
	...
	IF(ISNULL(@StatusCode,0) >=200) BEGIN
		...
		SELECT TMD.InterfaceRefID FROM TestMaterialDetermination TMD
		JOIN @TVP_TMD_WithDate TT ON TT.MaterialID = TMD.MaterialID AND TMD.DeterminationID = TT.DeterminationID AND ISNULL(TT.Selected,0) = 1 AND TMD.TestID = @TestID AND ISNULL(TMD.StatusCode,0) <> 500
		GROUP BY TMD.InterfaceRefID;
	...
	BEGIN
		...
		INSERT(TestID,MaterialID,DeterminationID,ExpectedDate,StatusCode) VALUES (@TestID,S.MaterialID,s.DeterminationID,S.ExpectedDate,100);
...
INSERT INTO [Status] ([StatusID], [StatusTable] ,[StatusCode] ,[StatusName] ,[StatusDescription])
     VALUES
     (48, 'TestMaterialDetermination', 500, 'Cancelled in LIMS', 'Cancelled in LIMS')
GO