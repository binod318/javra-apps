...
-------------------------------------------------------------------
==============================Example==============================
EXEC PR_RDT_GetResultForExcel 10622, 0
*/
									...
									(
										...
										(
											SELECT 
												...
												MappingColumn,
												Score = CASE 
															WHEN ISNULL(TR.MappingColumn,'''') = ''resistantper'' AND ISNULL(TR.[Percentage],0) <> 0 THEN CAST(TR.[Percentage] as nvarchar(10))
															WHEN ISNULL(TR.MappingColumn,'''') = ''resistantper'' AND ISNULL(TR.[Percentage],0) = 0 THEN TR.Score
															WHEN ISNULL(TR.MappingColumn,'''') = ''SusceptibilityPer'' AND ISNULL(TR.Score,'''') <> '''' THEN TR.Score
															WHEN ISNULL(TR.MappingColumn,'') = ''SusceptibilityPer'' AND ISNULL(TDR.TraitResult,'''') <> '''' THEN TDR.TraitResult
											...
											JOIN RDTTraitDetResult TDR ON TDR.RelationID = RTD.RelationID
																			AND (CASE
																					WHEN (TR.MappingColumn = ''SusceptibilityPer'' AND TDR.MappingCol = ''SusceptibilityPer'' AND (ISNULL(TR.Score,'''') <> '' OR TR.[Percentage] BETWEEN TDR.MinPercent AND TDR.MaxPercent)) THEN 1
																					WHEN (TR.MappingColumn = ''resistantper'' AND TDR.MappingCol = ''resistantper'' AND (ISNULL(TR.[Percentage],0) <> 0 OR ISNULL(TR.Score,'''') <> '''')) THEN 1
