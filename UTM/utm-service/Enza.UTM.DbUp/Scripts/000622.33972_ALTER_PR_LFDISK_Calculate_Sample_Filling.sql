...
AS BEGIN
	...
	BEGIN
		...
		SELECT 
			TM.TestMaterialID, 
			MaterialName = COALESCE(T3.origin, T3.Femalecode, ''), 
			NrOfPlants
		FROM TestMaterial TM
		JOIN 
		(
			SELECT T2.TestMaterialID, T2.RowID, T2.origin, T2.[Female code] AS Femalecode
					FROM
					(
						SELECT 
							T.TestID,
					...
					(
						Max(CellValue)
						FOR [ColumLabel] IN ([origin], [Female code])
					) T2
