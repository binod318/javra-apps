...
BEGIN
	...
	BEGIN TRY
		...
		IF(ISNULL(@Isolated,0) = 0) BEGIN
			SELECT TOP 1 @DNATypeTestProtocolID = MTP.TestProtocolID 
			FROM MaterialTypeTestProtocol MTP
			JOIN TestProtocol TP ON TP.TestProtocolID = MTP.TestProtocolID
			WHERE MaterialTypeID = @MaterialTypeID AND CropCode = @CropCode AND TP.TestTypeID = @TestTypeID;
		END
		ELSE BEGIN
			SELECT TOP 1 @DNATypeTestProtocolID = TestProtocolID 
			FROM TestProtocol
			WHERE Isolated = 1 AND TestTypeID = @TestTypeID;
		END
