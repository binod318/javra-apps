...
Krishna Gautam		2022-04-06			#34290:Send variety instead of e-number while printing
...
AS BEGIN
	...
	BEGIN
		...
		SELECT TMD.InterfaceRefID AS LimsID, TM.MaterialStatus, TMD.NrPlants, D.DeterminationName, M.MaterialKey, GID, [Plant Name], lotNr,Variety, MasterNr , T.ImportLevel, T.LabPlatePlanName
		FROM Test T 
		...
		(
			...
			(
				SELECT 
					T1.DeterminationID,
					ColumnLabel = MAX(T.ColumnLabel),
					ColumnNr	= MAX(T2.ColumnNr)
		...
		) D ON D.DeterminationID = TMD.DeterminationID
		JOIN 
		(
			SELECT T2.MaterialKey, T2.[Plant name], T2.[GID], T2.lotNr, T2.Variety,T2.MasterNr
			FROM
			(
				SELECT 
					R.MaterialKey,
				...
				JOIN Test T ON T.FileID = F.FileID
				WHERE C2.ColumLabel IN('Plant name','GID', 'lotNr', 'Variety', 'MasterNr')
				AND T.TestID = @TestID
			) T1
			PIVOT
			(
				Max(CellValue)
				FOR [ColumLabel] IN ([Plant name], [GID], [lotNr],[Variety],[MasterNr])
			) T2

		) PT ON PT.MaterialKey = M.MaterialKey 
		WHERE T.TestID = @TestID AND TMD.TestID = @TestID AND TM.TestID = @TestID
				AND TMD.StatusCode = 400
	...
	BEGIN
		SELECT TMD.InterfaceRefID AS LimsID, TM.MaterialStatus, TMD.NrPlants, D.DeterminationName, M.MaterialKey, GID, [Plant Name], lotNr, Variety, MasterNr, T.ImportLevel, T.LabPlatePlanName 
			...
			WHERE 
				T.TestID = @TestID AND TMD.TestID = @TestID AND TM.TestID = @TestID
				AND TMD.StatusCode = 400
			ORDER BY R.RowID, D.ColumnNr
	END
END
