...
EXEC PR_RDT_GetMaterialWithTests 10638,1, 150, ''
...
AS BEGIN
	...
	DECLARE @DetColumns NVARCHAR(MAX),@DetColumnIDs NVARCHAR(MAX), @ImportedColumns NVARCHAR(MAX), @ImportedColumnIDs NVARCHAR(MAX), @AllColumns NVARCHAR(MAX),@DateColumns NVARCHAR(MAX), @LimsIDColumns NVARCHAR(MAX), @NrOfPlantsColumns NVARCHAR(MAX);

	DECLARE @LimsIDPivotQuery NVARCHAR(MAX) =  '', @NrOfPlantsPivotQuery NVARCHAR(MAX);
	...
	SELECT @FileID = F.FileID,@ImportLevel = T.ImportLevel, @TestStatusCode = T.StatusCode
	FROM [File] F
	...
	SELECT 
		ColumnID, CONCAT('D_', TraitID), CONCAT(ColumnLabel , ' (', DeterminationCount ,')'), 4, ColumnNr, 1, 'Bool'		
	FROM @TblTempColumn
	order by ColumnNr;
	...
	INSERT INTO @TblColumns(ColumnID, TraitID, ColumnLabel, ColumnType, ColumnNr, Updatable,DataType)
	SELECT 
		ColumnID, CONCAT('Date_', TraitID), CONCAT(ColumnLabel , ', Exp date'), 3, ColumnNr, 1, 'Date'		
	FROM @TblTempColumn
	order by ColumnNr;
	...
	IF(ISNULL(@TestStatusCode,0) >= 500)
	BEGIN
		INSERT INTO @TblColumns(ColumnID, TraitID, ColumnLabel, ColumnType, ColumnNr,Updatable,DataType)
		SELECT TraitID,    CONCAT('limsRefID',TraitID), ColumnLabel+', LimsID', 2, ColumnNr, 1, 'NVARCHAR(MAX)' FROM @TblTempColumn ORDER BY ColumnNr;

	END

	--insert NrPlants column
	IF(ISNULL(@TestStatusCode,0) >= 500 AND ISNULL(@ImportLevel,'') <> 'PLT')
	BEGIN
		INSERT INTO @TblColumns(ColumnID, TraitID, ColumnLabel, ColumnType, ColumnNr,Updatable,DataType)
		SELECT TraitID,  CONCAT('nrofPlt', TraitID), CONCAT(ISNULL(ColumnLabel,''), ', NrPlts'),  1, ColumnNr, 0, 'NVARCHAR(MAX)' FROM @TblTempColumn ORDER BY ColumnNr;
