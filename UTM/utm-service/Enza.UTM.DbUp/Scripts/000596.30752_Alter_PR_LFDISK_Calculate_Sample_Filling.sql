...
AS BEGIN
	...
	BEGIN
		...
		SELECT 
			TM.TestMaterialID, 
			MaterialName = COALESCE(T3.Plantnumbr, T3.Femalecode), 
			NrOfPlants 
		FROM TestMaterial TM
		JOIN 
		(
			SELECT T2.TestMaterialID, T2.Plantnumbr, T2.[Female code] AS Femalecode
					FROM
					(
						SELECT 
							T.TestID,
					...
					(
						Max(CellValue)
						FOR [ColumLabel] IN ([Plantnumbr], [Female code])
					) T2
