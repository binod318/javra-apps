...
AS BEGIN
	...
	DECLARE @ColumnTable TABLE(ColumnID NVARCHAR(MAX), TraitID INT, Label NVARCHAR(MAX),[Order] INT, IsVisible BIT, Editable BIT, DataType NVARCHAR(MAX));
	...
	INSERT @ColumnTable(ColumnID, TraitID, Label, DataType, Editable, IsVisible, [Order])
	SELECT 
	   ColumnID, 
	   TraitID,
	   ColumLabel,
	...
	SELECT 
		@Columns  = COALESCE(@Columns + ',', '') +'CAST('+ QUOTENAME(MAX(ColumnID)) +' AS '+ MAX(Datatype) +')' + ' AS ' + ISNULL(QUOTENAME(TraitID), QUOTENAME(Label)),
		@Columns2  = COALESCE(@Columns2 + ',', '') + ISNULL(QUOTENAME(TraitID), QUOTENAME(Label)),
		@ColumnIDs  = COALESCE(@ColumnIDs + ',', '') + QUOTENAME(MAX(ColumnID))
	FROM @ColumnTable
	GROUP BY Label,TraitID
