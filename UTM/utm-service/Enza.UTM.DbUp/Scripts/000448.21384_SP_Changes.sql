	...
	BEGIN
		...
		JOIN RDTTraitDetResult TDR ON 
					...
					AND (CASE 
							...
							WHEN ISNULL(TR.[Percentage],0) <> 0  AND ISNULL(TDR.MappingCol,'') = ISNULL(TR.MappingColumn,'') THEN 1 --Resistant
...
-------------------------------------------------------------------
==============================Example==============================
EXEC PR_RDT_GetResultForExcel 10640, 1
*/
								...
								(
									...
									(
										...
										JOIN RDTTraitDetResult TDR ON TDR.RelationID = RTD.RelationID
																		AND (CASE 
																				WHEN TR.[Percentage] BETWEEN TDR.MinPercent AND TDR.MaxPercent AND ISNULL(TDR.MappingCol,'''') = ISNULL(TR.MappingColumn,'''') THEN 1 --Susceptibility
																				WHEN ISNULL(TR.[Percentage],0) <> 0  AND ISNULL(TDR.MappingCol,'''') = ISNULL(TR.MappingColumn,'''') THEN 1 --Resistance
