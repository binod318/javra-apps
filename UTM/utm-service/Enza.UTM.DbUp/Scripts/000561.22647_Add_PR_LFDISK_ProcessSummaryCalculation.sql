ALTER TABLE Test
ADD LDResultSummary NVARCHAR(100)
GO

IF NOT EXISTS (SELECT ConfigID FROM EmailConfig WHERE ConfigGroup = 'LD_TEST_COMPLETE_NEGATIVE')
BEGIN
	INSERT INTO EmailConfig(ConfigGroup, CropCode, Recipients)
	VALUES('LD_TEST_COMPLETE_NEGATIVE', '*', ''),
		  ('LD_TEST_COMPLETE_POSITIVE', '*', '')
...
BEGIN
	...
	('StatusCode','StatusCode',9,1,'string',0,1,0),
	('SiteName','Site',10,1,'string',0,1,0)
