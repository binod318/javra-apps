...
Binod Gurung		2022-04-19			#36608: Show only the test excluding RDT, SeedHealth and leaf disk.
========Example=============
	...
	SET @Query =	N';WITH CTE AS 
					(
						...
						(
							...
							WHERE F.CropCode IN  ('+@CropCodes +') AND 
									T.TestTypeID <=7 AND
									T.CreationDate >= DATEADD(YEAR,-1,GetDate()) AND 
									T.StatusCode >= 200 AND 
									ISNULL(T.BTR,0) = 0
						) T1 '+@Filter+'
		...
		SET @Query =	N';WITH CTE AS 
					(
						...
						(
							...
							WHERE F.CropCode IN  ('+@CropCodes +') 
									AND T.CreationDate >= DATEADD(YEAR,-1,GetDate()) 
									AND T.StatusCode >= 200 
									AND ISNULL(T.BTR,0) = 1
									AND T.TestTypeID <=7
						) T1 '+@Filter+'
					), COUNT_CTE AS (SELECT COUNT(TestID) AS TotalRows FROM CTE)
					SELECT CTE.*,
