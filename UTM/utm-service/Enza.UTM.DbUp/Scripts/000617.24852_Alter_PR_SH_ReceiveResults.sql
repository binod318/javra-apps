...
SET NOCOUNT ON;
	...
	SELECT SampleTestID, DeterminationID, [Key], [Value]
		...
		(
			...
			DeterminationID	NVARCHAR(MAX) '$.DeterminationID', -- Lims DeterminationID
			[Key]			NVARCHAR(50) '$.Key',
	...
	BEGIN TRY
		BEGIN TRANSACTION;
			...
			USING 
			(
				SELECT T1.SampleTestID, D.DeterminationID, T1.[Key], T1.[Value] FROM @TblScore T1
				JOIN Determination D ON D.OriginID = T1.DeterminationID AND D.Source = 'StarLims' -- DeterminationID from LIMS service has deteminationid of LIMS which is stored in OriginID in UTM
			) S ON S.SampleTestID = T.SampleTestID AND S.DeterminationID = T.DeterminationID AND S.[Key] = T.MappingColumn
