ALTER TABLE Test
ADD SampleConfigName NVARCHAR(100)
GO
...
========Example=============
EXEC PR_LFDISK_ImportFromExistingConfiguration 0, 12704,'binod.gurung@javra.com',1009,'ImportAgain2','',1,2
*/
...
AS BEGIN
    ...
    BEGIN TRY
		BEGIN TRANSACTION;
			...
			@SyncCode, CASE WHEN ISNULL(@ImportLevel,'') ='Plot' THEN 'Plot' ELSE 'CROSSES/SELECTION' END, @CountryCode, @TestProtocolID,@PlannedDate, @MaterialTypeID, @SiteID);
...
==================================Example======================================
--EXEC PR_LFDISK_SaveConfiguration 12799,'My custom config1'

*/


CREATE PROCEDURE [dbo].[PR_LFDISK_SaveConfiguration]
(
	@TestID INT,
...
(
    ...
    SELECT 
		ID = T.TestID,
		[Name] = T.SampleConfigName,
		F.CropCode,
		T.BreedingStationCode
	FROM Test T
	JOIN [File] F On F.FileID = T.FileID 
	WHERE T.TestTypeID = 9 AND ISNULL(T.SampleConfigName,'') <> '' AND YEAR(T.CreationDate) >= YEAR(GETDATE()) - 1 --AND T.StatusCode = 100
