...
const mapStateToProps = state => ({
  ...
  totalMarker: state.plateFilling.totMarker,
  reservePlateBtnDisable: state.plateFilling.reservePlateBtnDisable,
  total: state.plateFilling.total,

  warningFlag: state.plateFilling.reservePlates.warningFlag,
  warningMessage: state.plateFilling.reservePlates.warningMessage
});
const mapDispatchToProps = dispatch => ({
  ...
  confirmationNo: () => dispatch({ type: "RESERVE_PLATES_WARNING_FALSE" }),
  enableReservePlatesBtn: () => dispatch({ type: "RPBEnable" }),
  ...
  fetchFileList: (breeding, crop, testTypeMenu) => dispatch(filelistFetch(breeding, crop, testTypeMenu)),
  fetch_testLookup: (breedingStationCode, cropCode, freshFetch = false, testTypeMenu) => {
    dispatch(fetchTestlookup(breedingStationCode, cropCode, testTypeMenu));
  },
  fetch_well: testID => dispatch(fetchWell(testID)),
  fetch_total_marker: testID => {
    dispatch({ type: "REQUEST_TOTAL_MARKER", testID });
  },
  ...
  assign_fix_position: (testID, wellPosition, materialID) =>
    dispatch(assignFixPosition(testID, wellPosition, materialID)),
  clearPlateFilter: obj =>
    dispatch({ ...obj, type: "FETCH_CLEAR_PLATE_FILTER_DATA" }),
  ...
  completeRequest: (testId, statusCode) =>
    dispatch(testsLookupConfirmRequest(testId, statusCode)),
  ...
  reserveCall: (testID, forced) => dispatch(reserveCall(testID, forced)),
   ...
   */
  pagesizeForceChange: pageNumber =>
    dispatch({ type: "PAGE_PLATE_RECORD", pageNumber }),
  exportDNA: (testID, withControlPosition) => {
    dispatch({
      type: "REQUEST_PLATEFILLIG_EXCEL",
      testID,
      withControlPosition
    });
  }
