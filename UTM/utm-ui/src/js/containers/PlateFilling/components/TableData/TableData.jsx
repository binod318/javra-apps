...
class TableData extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      columnWidths: this._columnSize(props.columnList) || {},
      testID: props.testID,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.isBlocking !== this.props.isBlocking) {
      this.setState({ isBlocking: nextProps.isBlocking });
    }
  ...
  _columnSize = fields => {
    ...
    fields.map(d => {
      ...
      obj[d.traitID || d.columnLabel] = width < newwid ? newwid : width;
      return null;
    });
    return obj;
  };
  ...
  down = () => {
    ...
    if (this.state.dataList.length > position) {
      // console.warn(this.state.dataList.length);
      const data = this.props.dataList;
      const { selected } = this.state;
      const resultIndex = this.findIndex(data, selected, 1);
      this.props.move(selected, resultIndex);
      this.props.selectArrayChange(resultIndex);
      this.props.isBlockingChange(true);
      // console.info('resultIndex', resultIndex);
    }
  };
  moveBtn = () => {
  ...
  fetchFilterPlateData = () => {
    const { localFilter, pageSize, testID } = this.state;
    const obj = {
      testID,
      pageSize,
      filter: localFilter,
  ...
  render() {
    ...
    return (
      <div style={{ position: 'relative' }}>
        ...
        >
          {columns.map(d => {
            const width = columnWidths[d.traitID || d.columnLabel] || 80;
            const { traitID } = d;
              ...
              <Column
                ...
                header={
                  <FillingHeaderCell
                    ...
                    fetch_FilterPlate_data={this.fetchFilterPlateData}
...
const mapDispatchToProps = dispatch => ({
  move: (frm, to) => dispatch({ type: 'DATA_ROW_MOVE', frm, to }),
  deleteCall: (wellIDs, testID) =>
    dispatch({ type: 'REQUEST_DATA_DELETE', testID, wellIDs }),
  undoDead: (wellIDs, testID) =>
    dispatch({ type: 'REQUEST_UNDO_DELETE', wellIDs, testID }),
  ...
  undoFixedPosition: obj =>
    dispatch({ type: 'REQUEST_UNDO_FIXEDPOSITION', ...obj }),
  show_error: obj => dispatch(obj),
  addFilter: obj => {
