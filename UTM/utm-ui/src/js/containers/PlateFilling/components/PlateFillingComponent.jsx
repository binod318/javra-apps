...
class PlateFillingComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      autoFetchTimer: 30000, // 30000,

      importLevel: props.importLevel,
      totalMarker: props.totalMarker,
      reservePlateDisable: props.reservePlateBtnDisable,
  ...
  componentDidMount() {
    ...
    if (
      this.props.breedingStationSelected !== "" &&
      this.props.cropSelected !== ""
    ) {
    ...
    if (this.props.testID) {
      ...
      this.props.fetch_total_marker(this.props.testID);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.reservePlateBtnDisable !== this.props.reservePlateBtnDisable
    ) {
      this.setState({ reservePlateDisable: nextProps.reservePlateBtnDisable });
    }

    if (nextProps.testsLookup) {
      if (nextProps.testsLookup.length > 0 && nextProps.testID !== null) {
        const v = nextProps.testsLookup.filter(
          f => f.testID === nextProps.testID
        )[0];
        if (v) {
        ...
        this.setState({ testsLookup: nextProps.testsLookup });
      }
    }
    ...
    if (nextProps.statusCode !== this.props.statusCode) {
      ...
      // this.setState({ confirmDial: false });
      /**
      ...
      if (nextProps.statusCode !== 200) {
        ...
        // TODO :: fetch Data after status change from 200
        const obj = {
          testID: nextProps.testID,
          pageNumber: nextProps.pageNumber,
          pageSize:
            nextProps.testTypeSelected.wellsPerPlate || nextProps.pageSize,
          filter: nextProps.filter
        };
        if (this.props.testID === nextProps.testID) nextProps.plate_data(obj);
      }
    }
    ...
    if (
      nextProps.dataList.length !== this.props.dataList.length &&
      this.props.testID
    ) {
      this.props.fetch_total_marker(this.props.testID);
    }
    if (nextProps.totalMarker !== this.props.totalMarker) {
      this.setState({ totalMarker: nextProps.totalMarker });
    }
    ...
    if (nextProps.well) {
      this.setState({ well: nextProps.well });
    }
    ...
    if (nextProps.slotName !== this.props.slotName) {
      this.setState({ slotName: nextProps.slotName });
    }
  }
  ...
  onWellChange = (event, { newValue }) => {
    this.setState({ wellValue: newValue });
  };
  setIndexArray = index => {
    // index = null;
  ...
  handleWindowClose = e => {
    if (this.state.isBlocking) e.returnValue = "blocked";
  };
  lookupChange = e => {
    const { value } = e.target;
    // alert(value);
    this.updateDimensions();
    if (value) this.lookUpFetch(value, true);
  };
  lookUpFetch(value, isLookUpChanged = false) {
    const testType = this.props.testsLookup.find(d => {
      if (d.testID === value * 1) return d.testTypeName;
      return null;
    });

    this.props.fetch_well(testType.testID);
    this.props.fetch_total_marker(testType.testID);
    if (testType) {
      const obj = {
  ...
  toReservePlate = (statusCode, testID, forced) => {
    ...
    this.setState({ reservePlateDisable: true });

    this.props.testsLookup.map(test => {
      if (test.testID === testID) {
        if (!test.fixedPostionAssigned && !forced) {
          if (
            ...
            if (!checkStatus(statusCode, "REQUESTED")) {
              this.props.reserveCall(testID, forced);
            }
          }
        } else {
          this.props.reserveCall(testID, forced);
  ...
  toReturn = status => {
    // 7531 UTM: Delete message, combine buttons
    if (status) {
  ...
  toLims = e => {
    ...
    this.props.sendToLims(this.state.testID);
    // 7531 UTM: Delete message, combine buttons
    this.setState({ confirmDial: true });
  };
  ...
  blockingChange = status => {
    this.setState({ isBlocking: status });
  };
  wellFetchRequested = ({ value }) => {
    this.setState({ suggestions: wellSuggestions(value, this.state.well) });
  };
  wellClearRequested = () => {
  ...
  testSuggestionValue = value => {
    ...
    this.setState({ plantID: materialID });
    return materialKey;
  };
  testSuggestion = value => <div>{value.materialKey}</div>;
  fixPlantPosition = () => {
  ...
  exportDNA = testID => {
    if (this.props.testTypeSelected.wellsPerPlate === 92) {
      confirmAlert({
        title: "Confirm",
        ...
        buttons: [
          {
            ...
            onClick: () => this.props.exportDNA(testID, true)
          },
          {
            label: "No",
            className: "react-confirm-no",
            onClick: () => this.props.exportDNA(testID, false)
          }
        ]
      });
      return;
    }
    this.props.exportDNA(testID, true);
  };

  forceConfirm = choice => {
    const { confirmationNo } = this.props;
    const { statusCode, testID } = this.state;
    if (choice) {
      confirmationNo();
      this.toReservePlate(statusCode, testID, true);
    } else {
      confirmationNo();
      this.props.enableReservePlatesBtn();
    }
  };
  ...
  render() {
    ...
    const testIsList = importLevel && importLevel.toLowerCase() === "list";
    ...
    const greaterthan300less500 = statusCode >= 300 && statusCode < 500;
    const status350or300 = statusCode === 350 || statusCode === 300;

    //InLIMS for 2GB Reserved for DNA Isolation
    const excelExportBtn = (testTypeID === 1 && statusCode >= 500)  || (testTypeID === 2 && statusCode >= 300) ;
      ...
      <div className="plateFilling">
        ...
        >
          ...
          <div className="left limit570">
            ...
            {true && (
              <div className="form-e status-txt">
                <label className="full" title="Test">
                  Test
                  {": "}
                  {this.state.totalMarker}
                </label>
              </div>
            )}
          </div>
          <div className="right">
            ...
            {statusCode === 150 && (
              <button
                ...
                onClick={e => {
                  e.preventDefault();
                  this.toReservePlate(statusCode, testID, false);
                }}
                id="platefilling_reserve_plate_btn"
                disabled={this.state.reservePlateDisable}
              >
            ...
            {greaterthan300less500 && (
              <button
                title="To LIMS"
                className="with-i"
                onClick={this.toLims}
                disabled={this.state.confirmDial}
              >
...
PlateFillingComponent.propTypes = {
  exportDNA: PropTypes.func.isRequired,
  pagesizeForceChange: PropTypes.func.isRequired,
  fetch_total_marker: PropTypes.func.isRequired,
  totalMarker: PropTypes.number,
