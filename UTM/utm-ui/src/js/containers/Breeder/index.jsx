...
const mapState = state => ({
  ...
  update: state.breeder.error.update,
  forceUpdate: state.breeder.error.forceUpdate,
...
const mapDispatch = dispatch => ({
  ...
  plateTest: (plannedDate, cropCode, materialTypeID, isolated, expectedDate) =>
    dispatch(
      platesTestFetch(
        plannedDate,
        cropCode,
  ...
  clearPageData: () => dispatch({ type: "BREEDER_SLOT_PAGE_RESET" }),
  slotDelete: (slotID, cropCode, brStationCode, slotName) =>
    dispatch(slotDeleteAction(slotID, cropCode, brStationCode, slotName)),
  slotEdit: obj => dispatch({ type: "BREEDER_SLOT_EDIT", ...obj }),
  updateToFalse: () =>
    dispatch({ type: "BREEDER_UPDATE_FORCED", forceUpdate: false }),
  exportCapacityPlanning: payload => dispatch(exportCapacityPlanning(payload))
});
