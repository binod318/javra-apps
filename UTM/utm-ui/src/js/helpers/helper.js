...
const productionUrl = "utm.enzazaden.com";
export const isProduction =
  window.services.API_BASE_URL.indexOf(productionUrl) > -1;


export const hiddenTesttypes = () => {
  if(!isProduction)
    return "";

  let testTypes = window.services.HIDDEN_TEST_TYPE;

  if(testTypes.length > 0)
    testTypes = testTypes.toLowerCase();

  if(testTypes.indexOf("ldisk") > -1)
    testTypes = testTypes.replace('ldisk','ldisk|leafdisk');
