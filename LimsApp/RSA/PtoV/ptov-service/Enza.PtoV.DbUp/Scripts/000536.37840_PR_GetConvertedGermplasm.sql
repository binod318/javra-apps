DROP PROCEDURE IF EXISTS [dbo].[PR_GetConvertedGermplasm]
GO
...
EXEC [PR_GetConvertedGermplasm] 'ON', 1 ,100, @P1,0,'[StatusCode] IN (100)'
*/

CREATE PROCEDURE [dbo].[PR_GetConvertedGermplasm]
(
...
AS BEGIN
	...
	IF(@SendToVarmas = 0) BEGIN
		IF(@IsHybrid =1)
		BEGIN
			SET @ExcludeType =  ' WHERE CropCode = @CropCode AND TransferType NOT IN (''Female'',''Male'',''Maintainer'')'
		END
		ELSE
		BEGIN
									...
									(
										...
										(
											SELECT 
												...
												C.[ColumLabel],
												CASE 
													WHEN ISNULL(TSR.ScreeningValue,'''') = '''' THEN C1.[Value] 
													WHEN ISNULL(@SendToVarmas,0) = 1 THEN ISNULL(TSR.ScreeningValue,'''') -- For SendToVarmas
													ELSE CONCAT (ISNULL(TSR.ScreeningValue,''''), '' ('',C1.[Value] ,'')'') -- For display
												END AS CellValue,			
	...
	IF(ISNULL(@PGIDS,'') <> '') BEGIN
		...
		EXEC sp_executesql @Query, N'@SendToVarmas BIT, @FileID INT,@CropCode NVARCHAR(10)', @SendToVarmas, @FileID,@CropCode
	...
	ELSE BEGIN
		...
		C.ColumnLabel = CASE
							WHEN ISNULL(@SendToVarmas,0) = 1 THEN ISNULL(SF.SFColumnLabel,C.ColumnLabel) -- For SendToVarmas
							WHEN ISNULL(SF.SFColumnLabel,'') = '' THEN C.ColumnLabel ELSE CONCAT(SF.SFColumnLabel,' (', C.ColumnLabel,')') --For display
						END
		FROM @columnTable C
