...
AS BEGIN  
 ...
 UPDATE V SET   
  V.ENumber = CASE	WHEN T1.VarietyStatus = 'ACT' THEN T1.Enumber
					WHEN V.StatusCode <> 300 THEN T1.ENumber 
					ELSE V.ENumber END,  
  V.VarietyName = CASE	WHEN T1.VarietyStatus = 'ACT' THEN T1.VarietyName 
						WHEN V.StatusCode <> 300 THEN T1.VarietyName 
						ELSE V.VarietyName END,  
  V.StatusCode = CASE   
       WHEN T1.VarietyStatus = 'INACT' AND V.StatusCode <> 300 THEN 350
       WHEN T1.VarietyStatus = 'ACT' THEN  
        CASE   
         WHEN (V.ENumber <> T1.ENumber OR V.VarietyName <> T1.VarietyName OR V.VarmasStatusCode <> S.StatusCode) OR (V.StatusCode = 250) THEN 250   
         ELSE 200  
        END  
       ELSE V.StatusCode   
      END --set status to 250 if either Enumber or varietyname or varmasstatuscode is changed.  
	,V.VarmasStatusCode = CASE WHEN T1.VarietyStatus = 'ACT' THEN S.StatusCode
								WHEN V.StatusCode <> 300 THEN S.StatusCode 
								ELSE V.VarmasStatusCode END
...
--EXEC [PR_VtoPSync_GetGIDofVarieties] '615118,511606', 'NL' , 'ON'
ALTER PROCEDURE [dbo].[PR_VtoPSync_GetGIDofVarieties]  
(  
	@VarietyList NVARCHAR(MAX),
	@SyncCode NVARCHAR(10),
	@CropCode NVARCHAR(10) 
)  
AS 
BEGIN  
	...
	DECLARE @ObjectID INT, @ObjectType INT;

	SELECT @ObjectID = ObjectID, @ObjectType = ObjectType FROM [File] WHERE CropCode = @CropCode
	...
	FROM RelationPtoV R
	JOIN STRING_SPLIT(@varietyList,',') V ON CAST(V.[Value] AS INT) = R.VarietyNr
	LEFT JOIN Variety V1 ON V1.GID = R.GID
	WHERE R.StatusCode = 100 
	AND V1.GID IS NULL
