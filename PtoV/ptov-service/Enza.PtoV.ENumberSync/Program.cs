    ...
    class Program
    {
        ...
        static int Main(string[] args)
        {
            try
            {
                ...
                using (var operation = telemetryClient.StartOperation<RequestTelemetry>("EnumberSync"))
                {
                    ...
                    using (var scope = Container.BeginLifetimeScope())
                    {
                        ...
                        //var root1 = Path.Combine(Environment.CurrentDirectory, "Logs");
                        //var logFile1 = _logger.GetLogCurrentFile(root1);
                        //await SendErrorEmailAsync(emalConfigService, emailService, logFile1);
            ...
            try
            {
                ...
                if (tos.Any())
                {
                    await emailService.SendEmailAsync(tos,
                        "ENumberSync.exe execution error".AddEnv(),
        ...
        private static async Task SendDataErrorMailAsync(IEmailConfigService emalConfigService, IEmailService emailService, List<Entities.ExecutableError> dataError)
        {
            try
            {
                var cropCode = dataError.FirstOrDefault().CropCode;
                var config = await emalConfigService.GetEmailConfigAsync(EmailConfigGroups.ENUMBER_SYNC_DATA_ERROR, cropCode);
                var recipients = config?.Recipients;
                if (string.IsNullOrWhiteSpace(recipients))
                {
                    config = await emalConfigService.GetEmailConfigAsync(EmailConfigGroups.ENUMBER_SYNC_DATA_ERROR, "*");
                    recipients = config?.Recipients;
                }
                if (string.IsNullOrWhiteSpace(recipients))
                {
                ...
                if (tos.Any())
                {
                    ...
                    await emailService.SendEmailAsync(tos,
                        $"ENumberSync.exe execution error for Crop {cropCode}".AddEnv(),
                        message);
                }
            }
            catch (Exception ex)
