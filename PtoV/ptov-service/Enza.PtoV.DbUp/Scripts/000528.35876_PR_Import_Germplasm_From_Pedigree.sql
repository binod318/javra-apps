	...
	BEGIN TRY
		...
		BEGIN
			...
			(
				...
				JOIN @TVPColumn C1 ON C1.ColumLabel = C.ColumLabel
				JOIN @TVPCell CV ON CV.RowNr = R1.RowNr AND CV.ColumnNr = C1.ColumnNr
				WHERE F.FileID = @FileID
			) S ON S.RowID = T.RowID AND S.ColumnID = T.ColumnID
			--WHEN NOT MATCHED THEN INSERT (ColumnID, RowID, [Value])
			--			VALUES(S.ColumnID, S.RowID, S.[Value])
			WHEN MATCHED AND S.[Value] <> T.[Value] THEN UPDATE SET T.[Value] = S.[value] , T.Modified = CASE WHEN ISNULL(S.TraitID,0) <> 0 THEN 1 ELSE 0 END;
		END
