    ...
    class Program
    {
        ...
        static int Main(string[] args)
        {
            try
            {
                ...
                using (var operation = telemetryClient.StartOperation<RequestTelemetry>("ExternalLotSync"))
                {
            ...
            try
            {
                ...
                if (tos.Any())
                {
                    await emailService.SendEmailAsync(tos,
                        "ExternalLotSync.exe execution error".AddEnv(),
        ...
        private static async Task SendDataErrorMailAsync(IEmailConfigService emalConfigService, IEmailService emailService, List<Entities.ExecutableError> dataError)
        {
            try
            {
                ...
                var config = await emalConfigService.GetEmailConfigAsync(EmailConfigGroups.EXTERNALLOT_SYNC_DATA_ERROR, cropCode);
                var recipients = config?.Recipients;
                if (string.IsNullOrWhiteSpace(recipients))
                {
                    config = await emalConfigService.GetEmailConfigAsync(EmailConfigGroups.EXTERNALLOT_SYNC_DATA_ERROR, "*");
                    recipients = config?.Recipients;
                }
                if (string.IsNullOrWhiteSpace(recipients))
                {
                ...
                if (tos.Any())
                {
                    ...
                    await emailService.SendEmailAsync(tos,
                        $"ExternalLotSync.exe execution error for Crop {cropCode}".AddEnv(),
                        message);
                }
            }
            catch (Exception ex)
